...
def main():
    ...
    # # Get datetime
    # now = datetime.now()
    # time_cur = now.strftime("%d%b%Y %H-%M-%S")
    # datetime_cur = now.strftime("%Y-%M-%d %H-%M-%S")
    # timestamp_cur=int(now.timestamp())    
    # data_d["time_cur"]=time_cur
    # data_d["datetime_cur"]=datetime_cur
    # data_d["timestamp_cur"]=timestamp_cur
    
  
    # if data_d["cli_args"]["do_not_collect_mds_domains"]==False:
    #   log.info("--- 10. Get mds domains active")      
    #   get_mdss_domains_active_via_ssh.my_main(data_d, log) 
    # #exit()
            
            
    # log.info("--- 20. Get policies and targets for all MDSs domains via database query")  
    # get_all_dom_pol_targets.my_main(data_d, log)
    # # exit()
      

    # if data_d["cli_args"]["process_on_changed_policies"]==True:
    #   log.info("--- 40. Process_on_changed_policies")                
    #   process_on_changed_policies(data_d)

    # if data_d["cli_args"]["process_on_changed_targets"]==True:
    #   log.info("--- 50. Process_on_changed_targets")          
    #   process_on_changed_targets(data_d)      

    
    # log.info("--- 55. Update policies change windows in pol inst profile")          
    # mgmt_pol_install_profile_change_win.my_main(data_d, log)
    # #exit()

    # if data_d["cli_args"]["install_policies"]==True:               
    #   log.info("--- 60 Get install policies API calls")      
    #   get_install_policies_api_calls.my_main(data_d, log) 
              
    #   log.info("--- 70 Install policies")      
    #   install_policies.my_main(data_d, log)
